...
交付三个独立制品：systemd 只管理 `cc-connect-control`；control 通过私有 Unix Socket 监管 `cc-connect-server`；macOS launchd 管理 `cc-connect-runtime`。Runtime 使用出站 TLS/WebSocket 和 Ed25519 challenge-response 连接 control，并在本机复用 Codex 原生后端。服务器只保存不透明全局 `workspaceRef` 和最近目录 catalog，不复制 Codex 对话正文。
...
Release manifest 固定仓库 `shusfun/cc-connect`、workflow、tag、commit、六个制品及协议/数据库元数据。control 和 Runtime 均验证 GitHub OIDC/Sigstore identity、manifest 和 SHA-256，不提供未签名 fallback。
...
DeploymentManager 先锁定 Release 和机器级执行槽，再验签、检查活动 Turn/交互/realtime、检查磁盘、暂存在线 Runtime、备份 `control.db` 并写 activation record。进入 server stop 后运行不可取消；随后切换 `current` 并激活 Runtime，交给 systemd 启动候选 control。
...
多台 Mac 的项目和 thread 按设备隔离，离线设备保持只读 catalog 且不自动重发 Turn。更新时活动原生操作会明确阻断。验证覆盖签名与摘要、路径逃逸、数据库备份、handoff 两阶段恢复、Runtime 私有槽激活、执行槽、持久日志游标和候选确认。
